...
export async function POST(request: Request) {
  ...
  const row = {
    ...
    leagues: toArr(b.leagues),
    age_groups: toArr(b.age_groups),
    genders: toArr(b.genders),
    tryouts_open: !!b.tryouts_open,
    tryout_note: b.tryout_note || null,
    featured: !!b.featured,
    verified: !!b.verified,
  };

  const { error } = await service.from("clubs").upsert(row, { onConflict: "id" });
  if (error) {
    const msg = error.message.includes("clubs_slug_key")
      ? `The slug "${slug}" is already used by another club.`
      : error.message;
...
export async function DELETE(request: Request) {
  ...
  const { error } = await service.from("clubs").delete().eq("id", id);
