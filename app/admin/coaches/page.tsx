...
export default async function AdminCoachesPage() {
  if (!isSupabaseConfigured) return <Notice title="Admin needs Supabase">Add your Supabase keys to manage coaches.</Notice>;
  const admin = await getCurrentAdmin();
  if (!admin) {
    return (
      <Notice title="Admins only">
        Sign in with an admin account, then{" "}
        <Link href="/login?next=/admin/coaches" className="font-semibold text-brand-blue hover:underline">log in</Link>.
      </Notice>
    );
  }
    ...
    <>
      <section className="border-b border-slate-200 bg-navy py-10 text-white">
        <div className="container-page flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-heading text-3xl font-bold uppercase tracking-tight sm:text-4xl">Manage Coaches</h1>
            <p className="mt-1 text-slate-300">Add or edit real coaches — they appear live immediately.</p>
          </div>
          <div className="flex gap-2">
            <Link href="/admin" className="btn-outline text-sm">← Moderation</Link>
            <Link href="/admin/coaches/import" className="btn-amber text-sm">CSV import →</Link>
          </div>
        </div>
      </section>
      <div className="container-page py-8">
        <CoachManager coaches={coaches ?? []} regions={regions} />
      </div>
    </>
  );
}
