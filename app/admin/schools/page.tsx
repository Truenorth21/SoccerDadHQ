...
export default async function AdminSchoolsPage() {
  if (!isSupabaseConfigured) return <Notice title="Admin needs Supabase">Add your Supabase keys to manage schools.</Notice>;
  const admin = await getCurrentAdmin();
  if (!admin) {
    return (
      <Notice title="Admins only">
        Sign in with an admin account, then{" "}
        <Link href="/login?next=/admin/schools" className="font-semibold text-brand-blue hover:underline">log in</Link>.
      </Notice>
    );
  }
      ...
      <section className="border-b border-slate-200 bg-navy py-10 text-white">
        <div className="container-page flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <Link href="/admin/schools/import" className="btn-amber text-sm">CSV import →</Link>
          </div>
        </div>
      </section>
      <div className="container-page py-8">
        <SchoolManager schools={schools ?? []} regions={regions} />
      </div>
    </>
  );
}
