...
export default async function AdminClubsPage() {
  if (!isSupabaseConfigured) {
    return <Notice title="Admin needs Supabase">Add your Supabase keys to manage clubs.</Notice>;
  }
  ...
  if (!admin) {
    return (
      <Notice title="Admins only">
        ...
        <Link href="/login?next=/admin/clubs" className="font-semibold text-brand-blue hover:underline">log in</Link>.
      </Notice>
    );
  }
  const service = adminServiceClient();
  if (!service || !hasServiceKey) {
    return <Notice title="Service key required">Add <code>SUPABASE_SERVICE_ROLE_KEY</code> to manage clubs.</Notice>;
  }
  ...
  return (
    <>
      ...
      <div className="container-page py-8">
        <ClubManager clubs={clubs ?? []} regions={regions} />
      </div>
    </>
  );
}
