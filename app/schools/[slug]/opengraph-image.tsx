      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", fontSize: 26, color: "#e8a020", letterSpacing: 3, textTransform: "uppercase" }}>
              {school ? `${school.fhsaa_class} · ${regionName(school.region)}` : "Florida High School Soccer"}
            </div>
            <div style={{ display: "flex", fontSize: 60, fontWeight: 800, marginTop: 8, maxWidth: 920, lineHeight: 1.05 }}>
              {name}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", color: "#e8a020", fontWeight: 800, fontSize: 34 }}>
            {school ? `${school.mascot} · ★ ${school.rating.toFixed(1)}` : ""}
          </div>
